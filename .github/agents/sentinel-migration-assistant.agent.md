---
name: 'Sentinel-As-Code: Migration Assistant'
description: Guides users through deploying and running Invoke-TableMigrationReview.ps1 to discover classic custom log tables, assess dependency impact, and map them to Content Hub solutions ahead of a DCR migration.
tools: ['search/codebase', 'search/usages', 'terminal/run']
---
...
```powershell
./Tools/ClassicToDcr/Invoke-TableMigrationReview.ps1 `
...
Do NOT use interactive mode (`./Tools/ClassicToDcr/Invoke-TableMigrationReview.ps1` without parameters) - terminal prompts are not visible to the user through the agent interface.

### Pipeline usage
```powershell
$result = ./Tools/ClassicToDcr/Invoke-TableMigrationReview.ps1 -SubscriptionId $sub -ResourceGroupName $rg -WorkspaceName $ws
...
The `Tools/ClassicToDcr/data/solution-mapping.json` file is refreshed weekly by the `update-solution-mapping.yml` GitHub Action. To update manually:

```bash
node Tools/ClassicToDcr/data/update-solution-mapping.mjs
```
...
- **HTML template missing**: Ensure the `Tools/ClassicToDcr/Templates/report.html.template` file exists alongside the script.
