...
| `Rehearsal/New-DependencyFixture.ps1` | Rehearsal aid (in `Tools/ClassicToDcr/Rehearsal/`): builds known direct and indirect dependency chains (table, parser, analytics rule) so the assessment tool's detection can be proved against real resources. Analytics rules are created disabled | See [Tools/ClassicToDcr/README.md](../../Tools/ClassicToDcr/README.md) |
